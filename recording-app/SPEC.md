...
- **Stationary → auto-pause / Moving → auto-resume (GPS + accelerometer fusion)**: pause when speed < `pause_speed_kmh` (default 2) for > `pause_debounce_s` (default 3) and the accelerometer shows no motion. **Resume is triggered by the ACCELEROMETER first** — a rise in linear-acceleration variance above a threshold instantly resumes recording (GPS then confirms speed), because the accelerometer senses motion ~seconds before GPS speed lifts off zero and still works where GPS is poor (tunnels, urban canyons). Pausing finalizes the current segment; resuming starts a new one. Debounce prevents flapping at lights/junctions; saves disk *and* battery.
